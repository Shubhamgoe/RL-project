...
class SJSSP(gym.Env, EzPickle):
    ...
    def reset(self, data):   
        ...
        self.step_count = 0
        self.m = data[-1]                                
        self.dur = data[0].astype(np.single)                     # convert to float all numbers
        ...
        self.max_endTime = self.initQuality                                     # setting the baseline max end time
        self.finished_mark = np.zeros_like(self.m, dtype=np.single)             # matrix representing machine allocation showing no task has been allocated in the beginning
        # explanation
        # self.LBs = [[2, 5, 9], [1, 6, 12]]
        # self.finished_mark = [[0, 0, 0], [0, 0, 0]]
        # fea = np.concatenate((self.LBs.reshape(-1, 1), self.finished_mark.reshape(-1, 1)), axis=1)
        # # fea becomes:
        # # [[2, 0], [5, 0], [9, 0], [1, 0], [6, 0], [12, 0]]
        ...
        self.omega = self.first_col.astype(np.int64)                  # first tasks that can be executed
